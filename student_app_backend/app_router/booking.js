    ...
    wallet
      ...
      .then(async (walletres) => {
        ...
        if (walletres.current_amount > Number(data.total_price)) {
          ...
          const timestamp = "2023-03-29T18:25:00.252Z";
          const showdate = new Date(data.showTime);
          showdate.setMilliseconds(0);
          const formattedDate = showdate.toISOString();
          console.log("formattedDate", formattedDate);
          let created_on = new Date();
          created_on.setUTCHours(created_on.getUTCHours() + 5);
          created_on.setUTCMinutes(created_on.getUTCMinutes() + 30);
          let preparedata = {
            user_id: data.user_id,
            game_id: data.game_id,
            game_name: data.game_name,
            showTime: new Date(formattedDate),
            closeShowTime: new Date(data.closeShowTime),
            phone: data.phone,
...
router.get("/bookingList", async (req, res) => {
  try {
    ...
    if (query.referal_user_id !== "") {
      referalFillter["refered_user_id"] = await numberFunction.justNumbers(
        referal_user_id
      );
    }
    if (query.role_id !== "") {
