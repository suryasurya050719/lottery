...
export class LotteryResultComponent implements OnInit {
  ...
  Preview(newPage: any) {
    ...
    const isEmpty = Object.values(this.result_numerick).every((x) => x !== '');
    let resultNumberFormation = this.WinningNumberFormatter(
      this.result_numerick
    );
    console.log('resultNumberFormation', resultNumberFormation);
    const data = Object.values(JSON.parse(resultNumberFormation));
  ...
  WinningNumberFormatter(params: any) {
    const str = params;
    console.log('str', str);
